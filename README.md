...
A friendly, mobile-first Singapore restaurant bill splitter. Receipt recognition runs locally with Tesseract.js, and every result goes through editable review and deterministic integer-cent calculations.
...
- Receipt scanning lazy-loads Tesseract.js in a browser Web Worker; receipt images and recognized text stay in the browser.
...
The single OCR pass requests Tesseract word bounding boxes. PSM 4 can emit the item-name and right-aligned amount columns as separate text blocks, so those words are deterministically reconstructed into visual rows by vertical overlap and then sorted left-to-right before parsing. This retains PSM 4's improved text coverage without a second recognition pass, duplicate evidence, or another image buffer. The UI remains independent of Tesseract's types.

The deterministic Singapore parser recognizes explicit item amounts and common subtotal, total, GST/tax, service charge, discount, voucher/promo, and rounding labels. It normalizes all line endings and Unicode whitespace and generates stable positional parsed IDs; it does not use locale, time, or mutable parser state. It ignores common metadata lines (dates, times, telephone/card/reference/receipt numbers, tender and change), does not assume tax/service rates, and never creates a price without a decimal amount in the OCR evidence. Missing money is represented as `null`, not zero. Decorated, unpriced option rows are retained as modifier metadata and do not become claimable items. Receipt-level and item-level reconciliation each have reconciled, mismatch, and incomplete states. Every field stays editable; reconciliation never silently changes an amount. **View detected text** exposes the reconstructed OCR rows.

Normal tests use a text OCR fixture and synthetic bounding boxes to test row reconstruction, parser behavior, and reconciliation without requiring a binary receipt image. Full browser OCR needs canvas/image decoding, WASM, and a downloaded language model and can vary across those runtimes, so it is intentionally a manual integration check rather than part of every Vitest run. To exercise the full image pipeline with the permitted real receipt image, keep a local copy outside the repository, start the app with `npm run dev`, choose that image, select **Offline Scan**, and verify its preprocessing fingerprint is `3f834411fcaf399dab1f31a66243187a93627430aed2d8f2d132d824e7fa4f4a`; then review the detected rows and structured values. Parser-only tests do not claim to execute Tesseract.
