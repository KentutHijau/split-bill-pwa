...
`BillRepository` supplies `list`, `get`, `save`, and `remove`. `IndexedDbBillRepository` is the local implementation and supports `Blob` receipt/QR images plus raw OCR text without base64 or localStorage. Local OCR has no analytics or receipt submission path. A future remote repository can keep the same application-facing contract, with subscription methods added separately for live changes.
