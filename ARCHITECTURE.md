...
Line classification has exclusive precedence: summary/adjustment, metadata/payment, probable item, then ignore. Known summaries are therefore never reconsidered as food. Explicit `Grand Total` outranks generic `Total`; payment lines cannot overwrite either. `Receipt.explicitSubtotalDetected` distinguishes the printed subtotal from the editable item sum. Receipt-level reconciliation is `printed subtotal + adjustments = grand total`; the separate item-level check is `sum(items) = printed subtotal`. Editing items updates only the latter, never silently overwriting the printed receipt value.
